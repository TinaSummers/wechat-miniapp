...
Page({
  ...
  getRenderData() {
    ...
    //         mainService.link(pathModel.mc_index, 1);
    //       });
    //       return
    //     }
  ...
  initData(data) {
    ...
    data.questions.forEach((item, key) => {
      ...
      item.questions.type == 4 && (question.type = 6) && (question.picker_mode = 'date');
          ...
          if (v.logic_type == 2) {
            ...
            if (v.config.mode == 2) {
              options.forEach((v2, k2) => {
                if (v.details[0].redirect_type == 4) {
                  v2.jump_need = 1;
                  v2.jump_type = 2;
  ...
  pickerChange(e) {
    ...
    if (item.picker_mode == 'selector') {
      this.setData({
        [`renderData.questions[${index}].value`]: item.options[value].value,
  ...
  submitHandleCb(e) {
    ...
    ajaxService.questionReport({
      ...
      let { data: { errcode, data, errmsg } } = res;
      if (errcode == 0) {
        // ajaxService.sendPoint({
        //   survey_id: configModel.surveyId,
        // }).then((res) => {
        //   let { data: { errcode, data, errmsg } } = res;
        //   if (errcode == 0) {
        //     mainService.modal('提交成功！', '提示', () => {
        //       mainService.link(pathModel.pt_point_detail, 1);
        //     });
        //   } else {
        //     mainService.modal(errmsg);
        //   }
